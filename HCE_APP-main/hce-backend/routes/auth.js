...
router.post(
    '/signup', [
        ...
        body('companionPhoneNumber')
        .if(body('role').equals('Patient'))
        .optional()
        .notEmpty()
        .withMessage('Companion phone number is required for patients'),
    ...
    async(req, res) => {
        ...
        try {
            ...
            // Find companion by phone number if provided
            let companionId;
            if (role === 'Patient' && companionPhoneNumber) {
                const companion = await User.findOne({
                    phoneNumber: companionPhoneNumber,
                    role: 'Companion'
                });
                if (companion) {
