...
class ApiService {
  ...
  Future<Map<String, dynamic>> getCompanion(String token, String companionId) async {
    ...
    try {
      ...
      return {'name': 'Unavailable', 'phoneNumber': 'Connection Error', '_id': companionId};
    }
  }
  ...
  Future<List<String>> getReadingDates(String token, [String? patientId]) async {
  ...
  Future<bool> checkEspHealth() async {
    ...
    try {
      final response = await _client.get(Uri.parse(url))
        .timeout(const Duration(seconds: 5)); // Shorter timeout for health check
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
  ...
  Future<List<Map<String, dynamic>>> getUnseenMessageSenders(String token) async {
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (data.isEmpty) {
          print('Warning: No unseen message senders were returned from the API');
        }
        
        return List<Map<String, dynamic>>.from(data);
      } else {
        print('Error fetching unseen message senders: HTTP ${response.statusCode}');
        throw Exception('Failed to fetch unseen message senders: ${response.body}');
      }
    } catch (e) {
