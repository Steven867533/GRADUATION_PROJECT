...
class WebSocketManager {
  ...
  static void safeDispose() {
    if (!safeToDispose && !disposalRequested) {
      print("WebSocketManager: Skipping disposal as it's not safe to dispose yet");
      return;
    }
  ...
  static void setupListener(void Function(dynamic) onMessage, void Function() onDone, void Function(dynamic) onError) {
...
class ProfilePageState extends State<ProfilePage> with SingleTickerProviderStateMixin {
  ...
  void dispose() {
    ...
    _heartAnimationController.dispose();
      ...
      print('ProfilePage: Skipping disposal for $_instanceId as it may be reused');
    }
  ...
  Future<void> _getCurrentLocation() async {
    ...
    try {
      ...
      safeSetState(() {  // Use safeSetState instead of setState
  ...
  Future<void> loadData() async {
    ...
    try {
      ...
      if (userProfileData != null && userProfileData['role'] == 'Companion' && userProfileData['patientId'] != null) {
        try {
          // Get the patient's data
          final patient = await ApiService().getCompanion(token, userProfileData['patientId']);
          if (!mounted) return;  // Check if still mounted after async call
          safeSetState(() {  // Use safeSetState instead of setState
            patientData = patient;
      ...
      if (userProfileData != null && userProfileData['role'] == 'Patient' && userProfileData['companionId'] != null) {
        try {
          final companion = await ApiService().getCompanion(token, userProfileData['companionId']);
          if (!mounted) return;  // Check if still mounted after async call
          safeSetState(() {  // Use safeSetState instead of setState
      ...
      if (userProfileData != null && userProfileData['role'] == 'Patient' && userProfileData['doctorId'] != null) {
        try {
          final doctor = await ApiService().getCompanion(token, userProfileData['doctorId']);
          if (!mounted) return;  // Check if still mounted after async call
          safeSetState(() {  // Use safeSetState instead of setState
          doctorData = doctor;
        });
      ...
      final longitude = currentPosition?.longitude ?? 31.233334;
        final locations = await ApiService().getNearbyLocations(token, latitude, longitude);
        if (!mounted) return;  // Check if still mounted after async call
        safeSetState(() {  // Use safeSetState instead of setState
      ...
      final endOfWeek = startOfWeek.add(const Duration(days: 6));
          ...
          if (userData!['role'] == 'Companion' && patientId != null) {
            ...
            readings = await ApiService().getReadings(token, startOfWeek, endOfWeek, patientId);
          } else {
            // For patients, get their own readings
            readings = await ApiService().getReadings(token, startOfWeek, endOfWeek);
          }
  ...
  Future<void> _loadReadingsForDate(DateTime date) async {
    ...
    try {
      ...
      if (userData != null && userData!['role'] == 'Companion' && patientId != null) {
        // For companions, get readings for their patient using patientId
        readings = await ApiService().getReadings(token, startOfDay, endOfDay, patientId);
      } else {
        // For patients, get their own readings
        ...
        try {
          ...
          if (data['event'] == 'connected') {
            ...
            if (isMeasurementActive) {
              ...
              ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text(data['message'] ?? 'Finger removed from sensor')),
              );
            }
          } 
            ...
            safeSetState(() {
              ...
              if (data['finger_present'] != null && data['finger_present'] == false && isMeasurementActive) {
                ScaffoldMessenger.of(context).showSnackBar(
                  const SnackBar(content: Text('Please place your finger on the sensor')),
                );
              }
            });
          }
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Measurement complete! Reading saved.')),
            );
          }
            });
          }
  ...
  void _startWebSocketMeasurement() {
    if (!WebSocketManager.isConnected || WebSocketManager.channel == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('WebSocket not connected. Please try again.')),
      );
      return;
    }
  ...
  Future<void> _saveReadingToBackend() async {
    ...
    if (currentPosition == null) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Location not available. Reading saved without location.')),
      );
    }
  ...
  Future<void> startReading() async {
    ...
    if (!_isWebSocketConnected) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('WebSocket not connected. Please use the Connect button to establish a connection.'),
    ...
    if (_isWebSocketConnected) {
      ...
      try {
        ...
        setState(() {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Measurement started. Please wait...')),
          );
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(content: Text('Error starting measurement: ${response['message']}')),
          );
        }
      } catch (e) {
        ScaffoldMessenger.of(context).showSnackBar(
    ...
    Timer.periodic(pollInterval, (timer) async {
      ...
      try {
        ...
        if (resultResponse['status'] == 'success') {
          ...
          timer.cancel();
        } else if (!resultResponse['measurement_active'] && !resultResponse['server_busy']) {
          ...
          if (mounted) {
            ...
            ScaffoldMessenger.of(context).showSnackBar(
              const SnackBar(content: Text('Measurement was canceled. Please try again.')),
            );
          }
  ...
  Future<void> _verifyConnectionStatus() async {
    ...
    if (WebSocketManager.isConnected) {
      ...
      if (!isActive && WebSocketManager.isConnected) {
        print('Connection appears dead but was marked as connected - reconnecting');
        WebSocketManager.isConnected = false;
    ...
    else if (!WebSocketManager.isConnected && isHardwareConnected && !_isConnectingWebSocket) {
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(
        ...
        title: Text(
          userData != null && userData!['role'] == 'Companion' && patientData != null
              ...
              : SingleChildScrollView(
                  ...
                  child: Column(
                    ...
                    children: [
                      GestureDetector(
                        ...
                        child: Row(
                          children: [
                            ...
                            ElevatedButton.icon(
                              onPressed: _isConnectingWebSocket ? null : _connectWebSocket,
                              ...
                              label: Text(_isWebSocketConnected ? 'Reconnect' : 'Connect'),
                              style: ElevatedButton.styleFrom(
                                backgroundColor: _isWebSocketConnected ? Colors.blue.shade700 : Colors.blue,
                              ),
                            ),
                          ],
                        ),
                      ...
                      ElevatedButton(
                        onPressed: () async {
                          ...
                          if (datesWithData.isEmpty) {
                            ScaffoldMessenger.of(context).showSnackBar(
                              const SnackBar(content: Text('No dates with readings available')),
                            );
                            return;
                          }
                        ...
                        child: Row(
                          children: [
                            Stack(
                              ...
                              children: [
                                // Shadow effect
                                if (lastBeatTime.isNotEmpty && isMeasurementActive)
                                  Container(
                                    width: 50,
                                    height: 50,
                                    decoration: BoxDecoration(
                                ...
                                AnimatedContainer(
                                  duration: const Duration(milliseconds: 200),
                                  transform: Matrix4.identity()..scale(_heartScale),
                                  child: Icon(
                                    Icons.favorite,
                                    color: _isWebSocketConnected && isMeasurementActive 
                                        ? Colors.red 
                          ...
                          child: LineChart(
                            LineChartData(
                                ...
                                bottomTitles: AxisTitles(
                                  sideTitles: SideTitles(
                                    ...
                                    getTitlesWidget: (value, meta) {
                                      ...
                                      if (index >= 0 && index < selectedDateReadings.length) {
                                        DateTime time = DateTime.parse(selectedDateReadings[index]['timestamp']);
                                        return Text(
                                    ...
                                    interval: selectedDateReadings.length > 8 
                                        ? (selectedDateReadings.length / 4).ceil().toDouble() 
                                        : 1,
                                    reservedSize: 28,
                                  ),
                                ),
                              ...
                              lineBarsData: [
                                ...
                                LineChartBarData(
                                  spots: _generateDailyChartData().where((spot) => 
                                    !isNormalHeartRate(spot.y)).toList(),
                                  isCurved: false,
                              ...
                              maxX: selectedDateReadings.isEmpty 
                                  ? 1 
                                  : (selectedDateReadings.length - 1).toDouble(),
                              ...
                              title: Row(
                                children: [
                                  Text(
                                    ...
                                    style: TextStyle(
                                      fontWeight: FontWeight.bold,
                                      color: isNormalHR ? Colors.black : Colors.red,
                                    ),
                                  ),
                                  ...
                                  Container(
                                    ...
                                    decoration: BoxDecoration(
                                      color: isNormalHR ? Colors.green.withOpacity(0.2) : Colors.red.withOpacity(0.2),
                                    ...
                                    child: Text(
                                      ...
                                      style: TextStyle(
                                        fontSize: 12,
                                        color: isNormalHR ? Colors.green[700] : Colors.red[700],
                                      ),
                                    ),
                                  ),
                                ],
                              ...
                              subtitle: Row(
                                children: [
                                  Text(
                                    ...
                                    style: TextStyle(
                                      color: isNormalSpO2 ? Colors.black87 : Colors.red,
                                    ),
                                  ),
                                  const SizedBox(width: 10),
                                  Container(
                                    padding: const EdgeInsets.symmetric(horizontal: 8, vertical: 2),
                                    decoration: BoxDecoration(
                                      color: isNormalSpO2 ? Colors.blue.withOpacity(0.2) : Colors.red.withOpacity(0.2),
                                      borderRadius: BorderRadius.circular(12),
                                    ),
                                    child: Text(
                                      ...
                                      style: TextStyle(
                                        fontSize: 12,
                                        color: isNormalSpO2 ? Colors.blue[700] : Colors.red[700],
                                      ),
                                    ),
                                  ),
                                  const Spacer(),
                      ...
                      if (userData != null && (userData!['role'] == 'Patient' || (userData!['role'] == 'Companion' && patientData != null))) ...[
                        const Text(
                          ...
                          child: LineChart(
                            LineChartData(
                              gridData: const FlGridData(
                                ...
                                horizontalInterval: 20, // Changed from 30 to 20 for better spacing
                              ),
                              titlesData: FlTitlesData(
                                bottomTitles: AxisTitles(
                                  sideTitles: SideTitles(
                              ...
                              maxY: _calculateWeeklyMaxY(bloodPressureData), // Use a modified function for the weekly chart
  ...
  double _calculateWeeklyMaxY(List<FlSpot> chartData) {
    ...
    double maxValue = validPoints.map((spot) => spot.y).reduce((a, b) => a > b ? a : b);
