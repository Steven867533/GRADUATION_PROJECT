...
class SignUpPageState extends State<SignUpPage> {
  ...
  Future<void> _signUp() async {
    ...
    if (!phoneController.text.startsWith('+')) {
      _showErrorMessage('Phone number should start with + (e.g., +20123456789)');
      return;
    }
    ...
    if (_selectedRole == 'Companion' && 
      ...
      _showErrorMessage('Patient phone number should start with + (e.g., +20123456789)');
      return;
    }
    ...
    if (_selectedRole == 'Patient') {
      if (companionPhoneController.text.isNotEmpty && 
        ...
        _showErrorMessage('Companion phone number should start with + (e.g., +20123456789)');
        return;
      }
      
      if (doctorPhoneController.text.isNotEmpty && 
          !doctorPhoneController.text.startsWith('+')) {
        _showErrorMessage('Doctor phone number should start with + (e.g., +20123456789)');
        return;
      }
    }
    ...
    final result = await authProvider.signUp(
      ...
      doctorPhoneNumber: _selectedRole == 'Patient' ? doctorPhoneController.text : null,
    );
    ...
    if (result['success']) {
      ...
      _showErrorMessage(result['message'] ?? 'Signup failed. Please try again.');
    }
  }
                ...
                if (_selectedRole == 'Patient')
                  TextField(
                    ...
                    decoration: const InputDecoration(
                      ...
                      hintStyle: TextStyle(fontSize: 18),
                      prefixIcon: Icon(Icons.medical_services, color: Colors.blue, size: 28),
                      border: OutlineInputBorder(
                        borderRadius: BorderRadius.all(Radius.circular(30)),
                        borderSide: BorderSide.none,
                      ),
