    ...
    try {
      ...
      if (data['event'] == 'connected') {
        ...
        if (!data['server_busy']) {
          ...
          ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(
                content: Text('Server is busy with another measurement')),
          );
        }
        ...
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
              content: Text(data['message'] ??
                  'Finger removed from sensor. Measurement canceled.')),
        );
        ...
        setState(() {
          ...
          if (currentHeartRate > 0) {
            ...
            realtimeHeartRateData
                .add(FlSpot(dataPointCount.toDouble(), currentHeartRate));
            ...
            if (currentSpO2 > 0) {
              ...
              realtimeSpO2Data
                  .add(FlSpot(dataPointCount.toDouble(), currentSpO2));
            }
          }
        });
        ...
        setState(() {
          isMeasurementActive = false;
          if (data.containsKey('final_heart_rate') &&
              data['final_heart_rate'] != null) {
        ...
        if (!_hasCurrentReadingBeenSaved) {
          ...
          print(
              'Ignoring duplicate measurement_complete event - reading already saved');
    ...
    try {
      ...
      if (userRole == 'Companion' && patientId != null) {
        ...
        readings = await ApiService()
            .getReadings(token, startDate, endDate, patientId);
      } else {
  ...
  Future<void> _loadPatientData() async {
    ...
    try {
      if (userProfile['role'] == 'Companion' &&
          userProfile['patientId'] != null) {
        // Get patient details
        final patient =
            await ApiService().getCompanion(token, userProfile['patientId']);
            ...
            children: [
              IconButton(
                icon: Icon(
                  ...
                  color: isConnected
                      ? Colors.green
                      : (isConnecting ? Colors.orange : Colors.white),
                ),
                            ...
                            children: [
                              ...
                              Text(
                                ...
                                style: TextStyle(
                                  fontSize: 16,
                                  color:
                                      fingerPresent ? Colors.green : Colors.red,
                                ),
                              ),
                            ],
                          ),
                          ...
                          child: realtimeHeartRateData.length < 2
                              ...
                              : LineChart(
                                  LineChartData(
                                    ...
                                    minX: realtimeHeartRateData.isEmpty
                                        ? 0
                                        : realtimeHeartRateData.first.x,
                                    maxX: realtimeHeartRateData.isEmpty
                                        ? 1
                                        : realtimeHeartRateData.last.x,
                                    minY:
                                        40, // Lower minimum to show more context
                                    maxY:
                                        140, // Higher maximum for potential high rates
                                  ),
                                ),
                        ),
              ...
              children: [
                ...
                IconButton(
                  icon: Icon(
                    Icons.arrow_right,
                    size: 40,
                    color: currentDayIndex < readingsData.keys.length - 1
                        ? Colors.blue
                        : Colors.grey,
                  ),
                  onPressed: currentDayIndex < readingsData.keys.length - 1
                      ? nextDay
                      : null,
                ),
              ],
                          ...
                          bottomTitles: AxisTitles(
                            sideTitles: SideTitles(
                              ...
                              getTitlesWidget: (value, meta) {
                                ...
                                if (index >= 0 &&
                                    index < currentReadings.length) {
                                  DateTime time =
                                      currentReadings[index]['time'];
                                  return Text(
                              ...
                              interval: chartData.length > 8
                                  ? (chartData.length / 4).ceil().toDouble()
                                  : 1,
                              reservedSize: 28,
                            ),
                          ),
                ...
                children: [
                  ...
                  ...currentReadings.asMap().entries.map((entry) {
                    ...
                    String heartRateStatus =
                        getHeartRateLabel(heartRate.toInt());
                    String spo2Status = getSpO2Label(spo2.toInt());
                    Color heartRateColor =
                        heartRateStatus == 'Normal' ? Colors.green : Colors.red;
                    Color spo2Color =
                        spo2Status == 'Normal' ? Colors.green : Colors.red;

                    // Combined status
                    String status =
                        heartRateStatus == 'Normal' && spo2Status == 'Normal'
                            ? 'Normal'
                            : 'Abnormal';
                    Color statusColor =
